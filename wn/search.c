...
static void findverbgroups(IndexPtr idx)
{
	 ...
	 for (j = 0; j < synset->ptrcount; j++) {
		   ...
		   for (k = 0; k < idx->off_cnt; k++) {
		       if (synset->ptroff[j] == (int) idx->offset[k]) {
...
static int getsearchsense(SynsetPtr synptr, int whichword)
{
	...
	for (i = 0; i < idx->off_cnt; i++)
	    if ((int) idx->offset[i] == synptr->hereiam) {
		free_index(idx);
		return(i + 1);
	    }
	free_index(idx);
...
static char *printant(int dbase, SynsetPtr synptr, int wdnum, char *templ, char *tail)
{
    int i, j, wdoff;
    SynsetPtr psynptr;
    char tbuf[WORDBUF];
	...
	if (synptr->ptrtyp[i] == ANTPTR && synptr->pfrm[i] == wdnum) {
		...
		if (psynptr->ptrtyp[j] == ANTPTR &&
			...
			first = 0;
		    sprintf(retbuf + strlen(retbuf), templ, tbuf);
		}
	    }
	    free_synset(psynptr);
	}
